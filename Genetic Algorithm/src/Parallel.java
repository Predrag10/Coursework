...
public class Parallel {
	...
	public static PlayerThread[] threadArray = new PlayerThread[Interface.players.length];
	public static boolean threadStarted = false;

	// Running algorithm
	public static void RunParallel() throws InterruptedException {
		for (int i = 0; i < threadArray.length; i++) {
			threadArray[i] = new PlayerThread(Interface.minMoves, Interface.players[i]);
		}
	...
	public static void NewGeneration(Player players[]) {
		...
		for (int i = 0; i < Interface.playerNum; i++)
			Interface.ranks[i].setText(i + 1 + ". " + Parallel.playersDistance[i]);
	}
